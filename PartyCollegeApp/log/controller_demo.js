...
.controller("loginController", function ($rootScope, $scope, $stateParams, Restangular, $location, $state, $ionicPopover, $http, cordovaService, $ionicPlatform, $ionicHistory, getDataSource, showAlert, $ionicPopup, userHelp) {
...
.controller("menuController", function ($rootScope, $timeout, $scope, $state, $stateParams, $http, $ionicNavBarDelegate, $ionicSideMenuDelegate, cordovaService, $ionicHistory, showAlert) {
    showAlert.hideLoading();
    ...
    $scope.goSideMenu = function (menu) {
        if (menu.param && menu.param != "") {
            $state.go(menu.state, { category: menu.param });
        }
        else {
            $state.go(menu.state);
        }
...
.controller("videoPlayGxyController", function ($scope, $http, getDataSource, $ionicHistory, $sce, $timeout, $rootScope, $ionicListDelegate, $stateParams, $ionicNavBarDelegate, Restangular, $location, getUser, $ionicScrollDelegate, $state, $ionicLoading, $ionicActionSheet, $ionicModal, showAlert) {
    $scope.videos = [];
    wx.getNetworkType({
        success: function (res) {
            var networkType = res.networkType; // 返回网络类型2g，3g，4g，wifi
            if (networkType != 'wifi') {
                showAlert.alert("您正在使用非wifi网络，继续播放将产生流量费用");
            }
        }
    });
    $scope.nowtab = "简介";
    ...
    $scope.load = function () {
        wx.hideOptionMenu();
        var id = $stateParams.id;
        getDataSource.getDataSource("getVideoGroup", { videoid1: id, userid1: $rootScope.user.userid, videoid2: id, userid2: $rootScope.user.userid, videoid3: id }, function (data) {
            $scope.videoInfo = data[0];
            //记录播放日志
            getDataSource.getDataSource("getHasVideoStudyLog", { userid: $rootScope.user.userid, videoid: $scope.videoInfo.info_id }, function (logCount) {
                if (logCount[0].logcount == 0) {
                    getDataSource.getDataSource("insertAPP_VIDEOSTUDY_LOG",
                    { userid: $rootScope.user.userid, playtime: new Date(), videoid: $scope.videoInfo.info_id }, function (data) {
                    });
                }
            });

            getDataSource.getDataSource("getVideoSubs", { id: id }, function (data) {
                $scope.videos = data;
                $scope.loadPlayer();
            });
        });

        getDataSource.getDataSource("updateVideoPlayCount", { id: id }, function (data) {
        });
    }();
    $scope.goSeek = function () {
        var nowPlayTime = localStorage[$stateParams.id];
        $scope.API.play();
        $timeout(function () {
            $scope.API.seekTime(nowPlayTime);

        }, 1000);
        $scope.lastPlayTime = undefined;
    }
    //时间转换
    ...
    $scope.onPlayerReady = function (API) {
        ...
        $timeout(function () {
            if (localStorage[$stateParams.id]) {
                //API.seekTime(localStorage[$stateParams.id]);

                $scope.lastPlayTime = formatSeconds(localStorage[$stateParams.id]);
            }
    ...
    $scope.showCollection = function () {
        if ($scope.videoInfo.collectioned > 0) {
            showAlert.alert("您已收藏该视频");
            return;
        }
        $scope.videoInfo.collectioned = 1;

        getDataSource.getDataSource("showVideoLikes", { userid: $rootScope.user.userid, videoid: $stateParams.id, liketype: 0 });
    }
    //点赞
    $scope.showLike = function () {
        if ($scope.videoInfo.liked > 0) {
            showAlert.alert("您已对该视频点过赞");
            return;
        }
        $scope.videoInfo.liked = 1;
        getDataSource.getDataSource("showVideoLikes", { userid: $rootScope.user.userid, videoid: $stateParams.id, liketype: 1 });
    }
    ...
    $scope.loadPlayer = function () {
        ...
        var mediaType = "video/mp4";
        //alert($scope.AppConfig.videoPlayLocalPath + $scope.form.videopath);
        $scope.nowSrc = $sce.trustAsResourceUrl($scope.AppConfig.videoPlayLocalPath + $scope.videos[0].videopath);
        $scope.config = {
            sources: [
                {
                    src: $scope.nowSrc,
            ...
            mediatype: "video/mp4",
            updatetime: function (ct, duration) {
                if ($scope.API.currentTime != 0) {
                    localStorage.setItem($stateParams.id, parseInt($scope.API.currentTime / 1000));
                }
            },
            theme: "../bower_components/videogular-themes-default/videogular.css"
            , plugins: {
                poster: $scope.AppConfig.loadVideoPicPath + $scope.videoInfo.title_pic,//"http://www.videogular.com/assets/images/videogular.png",// 
                analytics: {
                    category: "Videogular",
                    label: "Main",
                    events: {
    ...
    $(".videoPlay_nav .one").click(function () {
...
.controller("video_listgxyController", function ($scope, getDataSource, $rootScope, $state, $stateParams) {
    $scope.list = [];
    $scope.isLog = false;
    $scope.getZTVideo = function () {
        $scope.title = "专题";
        getDataSource.getDataSource("getZTVideosByZTID", { ztid: $stateParams.id }, function (data) {
            $scope.list = data;
        })
    }
    $scope.getLikeVideo = function () {
        $scope.title = "我的点赞";
        getDataSource.getDataSource("getLikesVideos", { userid: $rootScope.user.userid }, function (data) {
            $scope.list = data;
        });
    }
    $scope.getCollectionVideo = function () {
        $scope.title = "我的收藏";
        getDataSource.getDataSource("getCollectionVideos", { userid: $rootScope.user.userid }, function (data) {
            $scope.list = data;
        });
    }
    $scope.getSearchWord = function () {
        getDataSource.getDataSource("getVideoBySearch", { word: $stateParams.id }, function (data) {
            $scope.list = data;
        })
    }
    $scope.getLogs = function () {
        getDataSource.getDataSource("getVideoStudyLogs", { userid: $rootScope.user.userid }, function (data) {
            $scope.list = data;
        })
    }
    $scope.showDetial = function (item) {
        $state.go("videoPlayGxy", { id: item.info_id });
    }
    if ($stateParams.category == "zt") {
        $scope.getZTVideo();
    }
    else if ($stateParams.category == "like") {
        $scope.getLikeVideo();
    }
    else if ($stateParams.category == "collection") {
        $scope.getCollectionVideo();
    }
    else if ($stateParams.category == "search") {
        $scope.getSearchWord();
    }
    else if ($stateParams.category == "log") {
        $scope.getLogs();
        $scope.isLog = true;
    }
...
.controller("index_videoController", function ($scope, getDataSource, $rootScope, $ionicSlideBoxDelegate, $state, $ionicPlatform) {
    $scope.categorys = [];
    $scope.tops = [];
    $scope.files = [];
    $scope.zts = [];
    ...
    $scope.listCategory = "videos";
    $scope.goNewest = function () {
        $scope.listCategory = "videos";
        getDataSource.getDataSource("getNewestVideos", {}, function (data) {
            $scope.files = data;
        })
        $scope.getTopLast();
        $("#zx").addClass("active");
    ...
    //通知公告
    $scope.goTZGG = function () {
        $state.go("app.gxynotice");
    }
    ...
    $scope.goSearch = function () {
        ...
        $scope.topClass = "topClass";
    }
    //获取公告
    getDataSource.getDataSource("getJSGG", {}, function (data) {

        $scope.gg = data;
    });

    $scope.showZT = function () {
        $scope.listCategory = "zt";
        $scope.getZTTopLast();
        $("#zt").addClass("active");
    }
    getDataSource.getDataSource("getVideoZT", {}, function (data) {
        $scope.zts = data;
    });
    $scope.goTops = function (item) {
        if (item.type == "zt") {
            $state.go("app.video_listgxy", { category: "zt", id: item.id });
        }
        else {
            $state.go("videoPlayGxy", { id: item.info_id });
        }
    }
    $scope.goZT = function (item) {
        console.log(item);
        $state.go("app.video_listgxy", { category: "zt", id: item.id });
    }
    $scope.goCategory = function (item) {
        $(".gxy_nav .navs_box .one").each(function () {
            $(this).removeClass("active");
        });
        $("#" + item.info_id).addClass("active");
        var navOneIndex = $("#" + item.info_id).index();
        if (navOneIndex < 3) {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(0px, 0px, 0px) scale(1);");
        } else if (navOneIndex < 4) {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(-60px, 0px, 0px) scale(1);");
        } else if (navOneIndex < 5) {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(-130px, 0px, 0px) scale(1);");
        } else if (navOneIndex < 6) {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(-210px, 0px, 0px) scale(1);");
        } else if (navOneIndex < 7) {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(-300px, 0px, 0px) scale(1);");
        } else if (navOneIndex < 8) {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(-360px, 0px, 0px) scale(1);");
        } else {
            $(".gxy_nav .navs_box").parent().attr("style", "transform: translate3d(-403px, 0px, 0px) scale(1);");
        }
        if (item.title == "最新") {
            $scope.goNewest();
        }
        ...
        if (item.info_id) {
            $scope.checkTitle(item);
        }
        $scope.goSearch();
        $scope.topClass = "";
    }
    //$scope.setSelectNav = function () {
    //    $(".navs_box_open .btn").click(function () {
    //        var one = $(this).html();
    //        alert(one);
    //        $(".gxy_nav .navs_box .one").each(function () {
    //            alert((this).html());
    //            if ($(this).html() == one) {
    //                $(this).addClass("active");
    //                alert($(".gxy_nav .navs_box .one").scrollleft());
    //                $(".gxy_nav .navs_box").scrollleft();
    //            }
    //        });
    //    });
    //}
    ...
    getDataSource.getDataSource("getVideoCategory", {}, function (data) {
        $scope.categorys = data;
        $scope.searchCategory.push({ title: "最新" });
        $scope.searchCategory.push({ title: "专题" });
        angular.forEach(data, function (item) {
            $scope.searchCategory.push(item);
        })
        $scope.categorys[0].check = true;

        //getDataSource.getDataSource("getVideoByCategory", { menuid: $scope.categorys[0].info_id }, function (items) {
        //    $scope.files = items;
        //});
        setTimeout(function () {
            $(".navs_box .one").click(function () {
                $(".navs_box .one").attr("class", "one");
                $(this).attr("class", "one active");
            });
            var navs_box_width = 12;
            $(".navs_box .one").each(function () {
                navs_box_width += $(this).outerWidth();
            });
            $(".navs_box").outerWidth(navs_box_width);
        }, 500);
    });

    $scope.goSearchWord = function () {
        $state.go("videoSearch");
    }
    getDataSource.getDataSource("getNewestVideos", {}, function (data) {
        $scope.files = data;
    })
    // 最新课程
    $scope.getTopLast = function () {
        getDataSource.getDataSource("getTopVideo", {}, function (data) {
            $scope.tops = data;
    ...
    $scope.getTopLast();
    // 专题置顶课程
    $scope.getZTTopLast = function () {
        getDataSource.getDataSource("getZTTopVideo", {}, function (data) {
            $scope.tops = data;
            $ionicSlideBoxDelegate.update();
        });
    }
    ...
    // 分类课程
    $scope.getFLTopLast = function (i) {
        getDataSource.getDataSource("getTopVideoByCategory", { menuid: i.info_id, levelpath: i.fulllevelpath }, function (data) {
            $scope.tops = data;
            $ionicSlideBoxDelegate.update();
        });
    }
    ...
    $scope.goDetial = function (item) {
        $state.go("videoPlayGxy", {
            id: item.info_id
        });
    }
    $scope.checkTitle = function (item) {
        $scope.listCategory = "videos";
        angular.forEach($scope.categorys, function (i) {
            if (i.info_id == item.info_id) {
                i.check = true;
                getDataSource.getDataSource("getVideoByCategory", { menuid: i.info_id, levelpath: i.fulllevelpath }, function (items) {
                    $scope.files = items;
                });
                $scope.getFLTopLast(i);
            }
            else {
                i.check = false;
            }
...
.controller("studentListController", function ($rootScope, $http, $scope, $stateParams, Restangular, $ionicActionSheet, getDataSource, $state, showAlert, $ionicHistory) {
    ...
    getDataSource.getDataSource(["getXZ", "getAllStu", "getBzrXx", "getClassTeacher"], { classid: $scope.bcid }, function (data) {
...
.controller("VhallVideoTestController", function ($http, $timeout, $scope, getDataSource, showAlert, $interval) {
    ...
    s2j_onVideoPlay = function () {
        ...
        $interval.cancel(checkMaxTime);
        //以下定时，可以作为后台记录播放进度的方法，$scope.nowtime就是视频真正播放进度，回传到后台进行数据库存储
        //建议20秒一次回传
        ...
        //以下定时用来判断最大播放时间
        checkMaxTime = $interval(function () {
            $scope.nowtime = player1.j2s_getCurrentTime();
...
.controller("previewImgController", function ($scope, getDataSource, $stateParams, $ionicHistory) {
    ...
    var id = $stateParams.id;
    getDataSource.getDataSource("getMessageChatImg", { chatid: id }, function (data) {
        //console.log(data[0].messagecontent);
        $scope.img = "../formImg/" + data[0].messagecontent;
    });
