...
function GetuSinaEmotionsHt(s, type, id, fid, getImgUrl, setModelValue) {
    ...
    $.ajax( {
        ...
        success: function (response) {
            ...
            if (setModelValue)
                setModelValue(getImgUrl(s), type, id, fid);
        }
    });
}
...
//替换
function AnalyticEmotionCallback(s, type, id, fid, setModelValue) {
    if (typeof (s) != "undefined") {
        var sArr = s.match(/\[.*?\]/g);
        if (sArr) {
            for (var i = 0; i < sArr.length; i++) {
                if (uSinaEmotionsHt.containsKey(sArr[i])) {
                    ...
                    GetuSinaEmotionsHt(s, type, id, fid, AnalyticEmotion, setModelValue);
                }
            }
        }
    }
