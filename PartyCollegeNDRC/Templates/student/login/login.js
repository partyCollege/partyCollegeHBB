app.controller("loginController", ['$scope', '$state', '$rootScope', '$document', '$timeout', '$interval', 'getDataSource', 'DateService', 'SessionService', 'smsService', 'CommonService', 'CookieService', 'Base64', "$http"
    , function ($scope, $state, $rootScope, $document, $timeout, $interval, getDataSource, DateService, SessionService, smsService, CommonService, CookieService, Base64, $http) {
        ...
        $scope.registerError = {
            idx: 0,
            message: "",
            style: [false, false, false, false, false, false, false, false, false, false, false],
            reset: function () {
        ...
        $scope.showErrorMessage = function (message, idx,isshow) {
        	$scope.registerError.reset();
        	$scope.registerError.message = message;
        	$scope.registerError.idx = idx;
        	$scope.registerError.style[idx] = isshow;
        }
    	...
    	//监控下拉框值变动
        $scope.$watch("registerObj.provice", function (newVal) {
        	...
        	if (newVal && $scope.allCity) {
        		...
        		if ($scope.isuserchange) {
        			$scope.registerObj.city = $scope.currentCities[0].name;
        		}
        	}
        });
    	//监控下拉框值变动
        $scope.$watch("registerObj.city", function (newVal) {

        	if (newVal && $scope.currentCities) {
        		$scope.currentCounties = _.filter($scope.currentCities, { name: newVal })[0].area;
        		if ($scope.isuserchange) {
        			$scope.registerObj.area = $scope.currentCounties[0];
        		}
        	}
        });
        ...
        $scope.validateInfo = function (callback) {
        	$scope.formData = angular.copy($scope.registerObj);
        	$scope.formData.password1 = md5($scope.formData.password1);
        	$scope.formData.password2 = md5($scope.formData.password2);

        	getDataSource.getUrlData("../api/registervalidate", $scope.formData, function (data) {
                if (data.result) {
                    callback(data);
                } else {
        ...
        $scope.remoteCheckCellphone = function () {
        	getDataSource.getUrlData("../api/registerExistsCellphone", { cellphone: $scope.registerObj.cellphone }, function (data) {
        		if (data.length > 0) {
        			$scope.showErrorMessage("手机号码已被使用", 0,true);
        		} else {
        			$scope.showErrorMessage("", 0,false);
        		}
        	}, function (error) {
        ...
        $scope.remoteCheckLogname = function () {
        	getDataSource.getDataSource("selectSyAccountByLogname", { logname: $scope.registerObj.logname }, function (data) {
        		if (data.length > 0) {
        			$scope.showErrorMessage("登录名已被使用", 2, true);
        		} else {
        			$scope.showErrorMessage("", 2, false);
        		}
        	}, function (error) {
        ...
        $scope.inputCheckPassword = function (idx) {
        	if ($scope.registerObj.password1.length > 0 && $scope.registerObj.password2.length > 0 && $scope.registerObj.password1 != $scope.registerObj.password2) {
        		$scope.showErrorMessage("确认密码和密码输入不一致", idx, true);
        	} else {
        		$scope.showErrorMessage("", idx, false);
        	}
        }

        $scope.passwordInvalidMsgClear = function (idx) {
        	$scope.showErrorMessage("", idx, false);
        }
        //判断手机号码
        ...
        $scope.SaveRegister = function () {
        	$scope.validateInfo(function (data) {
        		if (data.result) {
        			CommonService.layAlert("注册成功");
        			$scope.goBackIndex();
        		} else {
        			CommonService.layErrorAlert("注册失败");
        		}
        	});
        }
